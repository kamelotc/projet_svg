 ...
 * @return 0 si le nombre a été récupéré avec succès, 1 sinon.
 */
...
int create_classic_shape(FILE *svg, char shape, int color[3]) {
    ...
    switch (shape) {
        case 'E': fprintf(svg, "<ellipse cx=\"%d\" cy=\"%d\" rx=\"%d\" ry=\"%d\" fill=\"rgb(%d,%d,%d)\"/>", width/2, height/2, width/2, height/2, color[0], color[1], color[2]); break;
        case 'R': fprintf(svg, "<rect width=\"%d\" height=\"%d\" x=\"0\" fill=\"rgb(%d,%d,%d)\"/>", width, height, color[0], color[1], color[2]); break;
...
int create_circle(FILE *svg, int color[3]) {
    ...
    fprintf(svg, "<circle cx=\"%d\" cy=\"%d\" r=\"%d\" fill=\"rgb(%d,%d,%d)\"/>", radius, radius, radius, color[0], color[1], color[2]);
    fprintf(svg, "\n</svg>");
    return EXIT_SUCCESS;
}
...
int create_line(FILE *svg, int color[3]) {
    ...
    fprintf(svg, "<line x1=\"%d\" y1=\"%d\" x2=\"%d\" y2=\"%d\" stroke=\"rgb(%d,%d,%d)\"/>", x1, y1, x2, y2, color[0], color[1], color[2]);
    fprintf(svg, "\n</svg>");
    return EXIT_SUCCESS;
}
...
* @return 1 si une erreur s'est produite, 0 sinon.
*/
int create_polyshape(FILE *svg, const char shape, int color[3]) {
    ...
    fprintf(svg, "<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"%d\" height=\"%d\" viewBox=\"0 0 %d %d\">\n    ", viewWidth, viewHeight, viewWidth, viewHeight);
    switch (shape) {
        case 'S': fprintf(svg, "<polyline fill=\"none\" stroke=\"rgb(%d,%d,%d)\" points=\"", color[0], color[1], color[2]); break;
        case 'P': fprintf(svg, "<polygon fill=\"none\" stroke=\"rgb(%d,%d,%d)\" points=\"", color[0], color[1], color[2]); break;
